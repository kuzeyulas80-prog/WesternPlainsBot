const { Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, AllowedMentionsTypes } = require('discord.js');
...
const CLIENT_ID = '153559291485854106';          // Bot Client ID (Önceki turda 1535592914858541066 olarak güncellenmişti, kodda da o ID'yi kullanmalısın, şimdilik bu ID'yi bıraktım.)
...
// Banner Görsel URL'si (image_29.png'nin Discord CDN'e yüklenmiş URL'si)
...
client.on('interactionCreate', async interaction => {
  if (interaction.isChatInputCommand()) {
    ...
    // 4. MANAGE-SESSION COMMAND
      ...
      const rolePingContent = sessionRole ? `<@&${sessionRole.id}>` : '';
      ...
      const embed = new EmbedBuilder()
        ...
        // İstenen resmi banner olarak ekliyoruz (boşluksuz ve birleşik hissi verir)
  ...
  // BUTTON INTERACTIONS
    ...
    if (interaction.customId === 'vote_session_btn') {
      ...
      if (currentVotes >= session.votesNeeded) {
        ...
        // Oylama kapandığında da embed'i aynı şekilde koruyarak güncelliyoruz
        ...
        // Resim setImage() ile eklendiği için güncellerken kaybolmaz, sadece alanları güncelledik.
